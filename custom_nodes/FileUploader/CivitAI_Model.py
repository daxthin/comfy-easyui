...
class CivitAI_Model:
    ...
    def __init__(self, model_id, model_version=None, download_chunks=None, max_download_retries=None, warning=True, debug_response=False):
    ...
    def details(self):
        ...
        self.valid_types = ["LORA", "LoCon", "Checkpoint", "TextualInversion"]
        self.type = a["type"]

        print(a["type"])
                                                
        if self.type == "LORA":
                    self.model_path = folder_paths.folder_names_and_paths["loras"][0][0]
                    self.model_paths = folder_paths.folder_names_and_paths["loras"][0]
        elif self.type == "Checkpoint":
                    self.model_path = folder_paths.folder_names_and_paths["checkpoints"][0][0]
                    self.model_paths = folder_paths.folder_names_and_paths["checkpoints"][0]
        elif self.type == "LoCon":
                    self.model_path = folder_paths.folder_names_and_paths["loras"][0][0]
                    self.model_paths = folder_paths.folder_names_and_paths["loras"][0]
        elif self.type == "TextualInversion":
            self.model_path = folder_paths.folder_names_and_paths["embeddings"][0][0]
            self.model_paths = folder_paths.folder_names_and_paths["embeddings"][0]
        ...
        if response.status_code == 200:
            ...
            CivitAI_Model.PROGRESS["version"] = self.version

            CivitAI_Model.PROGRESS["id"] = model_data["id"]
            CivitAI_Model.PROGRESS["type"] = model_data["type"]
                    ...
                    if version_id == self.version and files:
                        for file in files:
                            ...
                            if download_url == model_download_url:
                                ...
                                CivitAI_Model.PROGRESS["model_name"] = file.get('name')
    ...
    def download(self):
        ...
        # DOWNLAOD BYTE CHUNK
        CivitAI_Model.PROGRESS["current"] = 0
        CivitAI_Model.PROGRESS["total"] = 0
        CivitAI_Model.PROGRESS["completed"] = False

        def download_chunk(chunk_id, url, chunk_size, start_byte, end_byte, file_path, max_retries=30):
            retries = 0
            retry_delay = 5
                    ...
                    if response.status_code == 206:
                        with open(file_path, 'r+b') as file:
                            ...
                            for chunk in response.iter_content(chunk_size=chunk_size):
                                ...
                                retries = 0
                                # CivitAI_Model.PROGRESS = {"current": start_byte + downloaded_bytes, "total": end_byte}
                                CivitAI_Model.PROGRESS["current"] = start_byte + downloaded_bytes
                                CivitAI_Model.PROGRESS["total"] = end_byte
                                # print(start_byte + downloaded_bytes, end_byte)
        ...
        for i in range(self.num_chunks):
            ...
            download_chunk(i, self.download_url, self.chunk_size, start_byte, end_byte, save_path, self.max_retries)

        model_sha256 = CivitAI_Model.calculate_sha256(save_path)
        if model_sha256 == self.file_sha256:
            print(f"{MSG_PREFIX}Loading {self.type}: {self.name} (https://civitai.com/models/{self.model_id}/?modelVersionId={self.version})")
            print(f"{MSG_PREFIX}{self.type} SHA256: {model_sha256}")
            CivitAI_Model.PROGRESS = {
                    "current": 0, 
                    "total": 0,
                    "completed": True,
                    "type": "none",
            ...
            print(f"{MSG_PREFIX}Download finished")
            self.dump_file_details()
            return True
        else:
...
@server.PromptServer.instance.routes.post("/civitai_download")
async def Download(request):
    post = await request.json()
    civitai = CivitAI_Model(
        model_id=post["id"],
        # save_path=download_path,
        # model_paths=LORAS,
        # model_types=["LoCon", "LORA", "Checkpoint"],
        model_version=post["version"],
    ...
    threading.Thread(target=civitai.download, daemon=True).start()
...
@server.PromptServer.instance.routes.get("/civitai_download_progress")
async def download_progress(self):
    # post = await request

    return web.json_response(CivitAI_Model.PROGRESS)


# @server.PromptServer.instance.routes.get("/ws/civitai_download_progress")
...
#     while True:
#         print(CivitAI_Model.PROGRESS)
#         await ws.send_json(CivitAI_Model.PROGRESS)
